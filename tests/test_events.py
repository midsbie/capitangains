...
def _make_event(symbol: str, outcome: GapResolution) -> GapEvent:
    return GapEvent(
        symbol=symbol,
...
def test_event_recorder_collects_and_exposes_list_reference():
    ...
    event = _make_event("ABC", outcome=GapResolution.UNACKNOWLEDGED)
...
def test_event_recorder_record_many_and_clear():
    recorder = EventRecorder()
    events = [
        _make_event("XYZ", outcome=GapResolution.SYNTHESIZED),
        _make_event("LMN", outcome=GapResolution.UNACKNOWLEDGED),
    ]
